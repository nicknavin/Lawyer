...
package com.app.lawyer.customview;
...
public class AnimatedExpandableListView extends ExpandableListView {
     ...
     * the flag for a group_selected and calls notifyDataSetChanged to cause the ListView
     * to refresh all of it's view. This time however, depending on whether a
     * group_selected is expanded or collapsed, certain childViews will either be ignored
     * or added to the list.
     *
     * Knowing this, we can come up with a way to animate our views. For
     * instance for group_selected expansion, we tell the adapter to animate the
     * children of a certain group_selected. We then expand the group_selected which causes the
     ...
     * However since the adapter knows that we are animating a certain group_selected,
     * instead of returning the real views for the children of the group_selected being
     ...
     * adapter to stop animating the group_selected and call notifyDataSetChanged. Now
     * the ExpandableListView is forced to refresh it's views again, except this
     * time, it will get the real views for the expanded group_selected.
     *
     * So, to list it all out, when {@link #expandGroupWithAnimation(int)} is
     * called the following happens:
     *
     * 1. The ExpandableListView tells the adapter to animate a certain group_selected.
     * 2. The ExpandableListView calls expandGroup.
     * 3. ExpandGroup calls notifyDataSetChanged.
     * 4. As an result, getChildView is called for expanding group_selected.
     * 5. Since the adapter is in "animating mode", it will return a dummy view.
     * 6. This dummy view draws the actual children of the expanding group_selected.
     ...
     *    animating the group_selected and notifyDataSetChanged is called again.
     ...
     *    child views.
     *
     * For animating the collapse of a group_selected is a bit more difficult since we
     ...
     * 3. As an result, getChildView is called for expanding group_selected.
     * 4. Since the adapter is in "animating mode", it will return a dummy view.
     * 5. This dummy view draws the actual children of the expanding group_selected.
    ...
    /**
     * Expands the given group_selected with an animation.
     * @param groupPos The position of the group_selected to expand
     * @return  Returns true if the group_selected was expanded. False if the group_selected was
     *          already expanded.
     */
    @SuppressLint("NewApi")
    public boolean expandGroupWithAnimation(int groupPos) {
        ...
        if (groupFlatPos != -1) {
            ...
            if (childIndex < getChildCount()) {
                // Get the view for the group_selected is it is on screen...
                View v = getChildAt(childIndex);
                if (v.getBottom() >= getBottom()) {
                    // If the user is not going to be able to see the animation
                    // we just expand the group_selected without an animation.
                    // This resolves the case where getChildView will not be
                    // called if the children of the group_selected is not on screen

                    // We need to notify the adapter that the group_selected was expanded
    ...
    /**
     * Collapses the given group_selected with an animation.
     * @param groupPos The position of the group_selected to collapse
     * @return  Returns true if the group_selected was collapsed. False if the group_selected was
     *          already collapsed.
     */
    public boolean collapseGroupWithAnimation(int groupPos) {
        ...
        if (groupFlatPos != -1) {
            ...
            if (childIndex >= 0 && childIndex < getChildCount()) {
                ...
                if (v.getBottom() >= getBottom()) {
                    // If the user is not going to be able to see the animation
                    // we just collapse the group_selected without an animation.
                    // This resolves the case where getChildView will not be
                    // called if the children of the group_selected is not on screen
                    return collapseGroup(groupPos);
                }
            } else {
                // If the group_selected is offscreen, we can just collapse it without an
                // animation...
                return collapseGroup(groupPos);
            }
        }
        ...
        // child of the group_selected we are trying to collapse, then set that
        // as the first child position of the group_selected... see
    ...
    /**
     * Used for holding information regarding the group_selected.
     */
    private static class GroupInfo {
         ...
         * We save this information so that if the user collapses a group_selected
    ...
    public static abstract class AnimatedExpandableListAdapter extends BaseExpandableListAdapter {
        ...
        public final int getChildType(int groupPosition, int childPosition) {
            GroupInfo info = getGroupInfo(groupPosition);
            if (info.animating) {
                // If we are animating this group_selected, then all of it's children
                // are going to be dummy views which we will say is type 0.
                return 0;
            } else {
                // If we are not animating this group_selected, then we will add 1 to
        ...
        public final View getChildView(final int groupPosition, int childPosition, boolean isLastChild, View convertView, final ViewGroup parent) {
            ...
            if (info.animating) {
                // If this group_selected is animating, return the a DummyView...
                ...
                if (childPosition < info.firstChildPosition) {
                    ...
                    // this group_selected when the group_selected view is not visible but the
                    // children of this group_selected are. When notifyDataSetChanged
                    ...
                    // refreshed. Now the problem is, if a group_selected has 2 items
                    // and the first visible item is the 2nd child of the group_selected
                    // and this group_selected is collapsed, then the dummy view will be
                    // used for the group_selected. But now the group_selected only has 1 item
                    // which is the dummy view, thus when the ListView is trying
                    // to restore the scroll position, it will try to jump to
                    // the second item of the group_selected. But this group_selected no longer
                    // has a second item, so it is forced to jump to the next
                    // group_selected. This will cause a very ugly visual glitch. So
                ...
                for (int i = info.firstChildPosition; i < len; i++) {
                    ...
                    if (totalHeight < clipHeight) {
                        ...
                        // if this group_selected has too many views, we don't want to
